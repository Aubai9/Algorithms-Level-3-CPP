...
void deleteClientByAccountNumber(vector<stClient>& vClients, string AccNumber)
{
    char confirm = 'n';
    
    for (auto it = vClients.begin(); it != vClients.end(); ++it)
    {
        if (it->Account_Number == AccNumber)
        {
            cout << "\nClient Found! Are you sure you want to delete? (y/n): ";
            cin >> confirm;
...
int main()
{
    ...
    deleteClientByAccountNumber(vClients, Account_Number);
